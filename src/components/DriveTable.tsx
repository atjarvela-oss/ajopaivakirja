...
export const DriveTable: React.FC<DriveTableProps> = ({
  ...
  const filteredDrives = drives.filter((d) => {
    ...
    const matchesSearch =
      ...
      (d.teacherNotes && d.teacherNotes.toLowerCase().includes(searchTerm.toLowerCase()));
    return matchesEnv && matchesSearch;
  });

  const handleDelete = (driveId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (confirm('Haluatko varmasti poistaa tämän ajokerran?')) {
      deleteLocalDrive(driveId);
      onRefresh();
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredDrives.map(d => {
      ...
      const dist = `Maantie:${d.environment.distribution.maantie}% Taajama:${d.environment.distribution.taajama}% Kaupunki:${d.environment.distribution.kaupunki}% Pysäköinti:${d.environment.distribution.pysakointi}%`;

      return [
        `"${s.toLocaleDateString('fi-FI')}"`,
        `"${s.toLocaleTimeString('fi-FI', { hour: '2-digit', minute: '2-digit' })}"`,
        `"${e.toLocaleTimeString('fi-FI', { hour: '2-digit', minute: '2-digit' })}"`,
        Math.round(d.durationSeconds / 60),
        `"${(d.durationSeconds / 3600).toFixed(2).replace('.', ',')}"`,
    ...
    link.href = url;
    link.download = `ajopaivakirja_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div id="driving-log-table-container" className="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
              Ajokertojen taulukko ({filteredDrives.length})
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Kaikki laitteeseen tallennetut ajotunnit
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="text-xs pl-9 pr-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white w-36 sm:w-48 focus:ring-2 focus:ring-blue-500 outline-hidden"
            />
          </div>
          ...
          <button
            ...
            className="px-2.5 sm:px-3 py-2 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 transition flex items-center space-x-1"
          ...
          {onExportPdf && (
            <button
              onClick={onExportPdf}
              className="px-3 py-2 text-xs font-bold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white shadow-xs transition flex items-center space-x-1"
              title="Luo virallinen PDF-ajopäiväkirja"
            >
          ...
          <button
            onClick={handlePrint}
            className="p-2 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 transition"
            title="Tulosta paperille"
          >
      ...
      {filteredDrives.length === 0 ? (
        <div className="text-center py-12 px-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
            Aloita ensimmäinen ajo GPS-seurannalla tai lisää ajokerta manuaalisesti.
          </p>
        </div>
      ) : (
